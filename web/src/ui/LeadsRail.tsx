 ...
 */
export function LeadsRail({index, selected, tab, onTab, onSelect, openCluster, onOpenCluster, insights, saved, data}: {
  ...
  /** Вкладка «Наблюдения»: временные шаблоны, кольца, устойчивость и честная проверка (features/insights). */
  insights?: ReactNode;
  ...
  return <nav className="wb-rail" aria-label="Очередь и кластеры">
    <div className="wb-rail__head"><div className="wb-segmented wb-rail__tabs" role="tablist" aria-label="Список">
      ...
      {insights && <button type="button" role="tab" aria-selected={tab === 'insights'} className={tab === 'insights' ? 'is-active' : undefined} onClick={() => onTab('insights')}>
        Наблюдения</button>}
    ...
    {tab === 'insights' && insights ? insights : tab === 'data' && data ? data : tab === 'saved' && saved ? saved : tab === 'leads'
