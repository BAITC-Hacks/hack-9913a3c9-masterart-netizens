...
цепочки и справку по счёту. Полный пересчёт на данных организаторов занимает секунды: первый
запуск в новой среде — около 1,7 с, повторный — меньше половины секунды.
...
просмотрщика, `run_receipt.json` с хешем входных файлов и временем расчёта и `last_attempt.json`
с итогом последней попытки. Если запуск завершится ошибкой, прежние выгрузки сохранятся.
...
**Измерено сейчас** (Apple M4 Max, Python 3.14): сам анализ занимает 0,3–0,4 с при повторном
запуске и около 1,7 с при первом запуске в новой среде, пиковая память — около 140 МБ; `analysis.json` весит 6,5 МБ, сборка интерфейса —
