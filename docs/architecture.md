...
каждой функции и её тесты — в [FEATURES.md](../FEATURES.md); то же самое простыми словами, на примере
одного счёта, — в [экскурсии по коду](code-tour.md).
...
## Импорт данных ([`imports/`](../imports))

| Модуль | Ответственность |
|---|---|
| [`ingest.py`](../imports/ingest.py) | Три файла parquet или CSV по схеме кейса → проверка размеров и имён → CSV читается с явными типами (идентификаторы — точные `int64`) → тот же загрузчик и тот же анализ → выгрузки заменяются только после полного успеха |
| [`README.md`](../imports/README.md) | Поддерживаемая схема, пределы и `POST /api/import` |
...
| [`features/shortlist/`](../web/src/features/shortlist) | Сохранённые счета: кнопка «Сохранить», вкладка «Сохранённые», PDF-справка по отмеченным |
| [`features/import/`](../web/src/features/import) | Загрузка своих данных (parquet или CSV) из окна «О данных» и вкладки «Данные» |
